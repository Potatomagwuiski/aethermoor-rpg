...
function cleanForge() {
    ...
    while ((match = regex.exec(content)) !== null) {
        ...
        // Remove trailing comma from preOutputs if it ended with one before outputs
        // actually preOutputs usually doesn't end with a comma, it ends with `],\n  `
        ...
        if (stats) {
            ...
            // NOTICE: no literal \n  } appended here!
            const newOutput = `outputs: { base: {${stats}} }${comma}`;
    ...
    console.log('Successfully wiped Gacha outputs from forge.ts FLAWLESSLY!');
}
