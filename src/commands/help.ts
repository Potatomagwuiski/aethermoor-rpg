...
export async function executeHelp(message: Message, args: string[]) {
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: '🌍 World & Navigation', value: '`rpg travel <zone>` - Journey to new zones (Lumina Plains, Whispering Woods, etc.) to access higher-tier monsters and ores. Gated by Level.' },
      { name: '⛏️ Gathering', value: '`rpg mine` - Extract Ores (Iron, Mythril).\n`rpg chop` - Fell Trees (Wood, Pine, Elderwood).\n`rpg fish` - Cast a line (Trout, Seaweed, Void Bass).\n`rpg harvest` - Harvest crops (Wheat, Potatoes, Moon Herbs).' },
